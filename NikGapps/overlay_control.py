...
for android_version in android_versions:
    repo_name = Statics.get_overlay_source_repo(args.android_version)
    repo_dir = Statics.get_overlay_source_directory(args.android_version)
    branch = "master"
    overlay_source_repo = Git(repo_dir)
    overlay_source_repo.clone_repo(repo_name, branch=branch)

    for overlay in NikGappsOverlays.get_overlay(android_version=args.android_version):
        overlay: Overlay
    ...
    if FileOp.dir_exists(repo_dir):
        overlays_repo_name = Statics.get_overlay_repo(args.android_version)
        overlays_repo_dir = Statics.get_overlay_directory(args.android_version)
