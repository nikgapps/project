...
run() {
  echo " "
  echo "for file in \"\$S/addon.d\"/*$fileName.sh; do"
  echo "  if [ -f \"\$file\" ]; then"
  echo "    if [ \"\$execute_config\" = \"0\" ]; then"
  echo "      ui_print \"- Deleting $fileName.sh\""
  echo "      rm -rf \"\$file\""
  echo "      rm -rf \"\$T/addon.d/\"*\"$fileName.sh\""
  echo "      exit 1"
  echo "    elif [ \"\$execute_config\" = \"1\" ]; then"
  echo "      CopyFile \"\$0\" \"\$S/addon.d/\$(basename \"\$file\")\""
  echo "    fi"
  echo "  elif [ \"\$execute_config\" = \"1\" ]; then"
  echo "    CopyFile \"\$0\" \"\$S/addon.d/\$(basename \"\$file\")\""
  echo "  fi"
